...
const isPrimitive = (
  value: number | string | Reference<number | string>
): boolean => ["string", "number"].includes(typeof value);

const getPath = (value: number | string | Reference<number | string>): string =>
  !isPrimitive(value) ? (value as Reference<number | string>).path : value;
...
export function isNotEqual(
  ...
  const defaultMessage = `\${path} must not be equal to ${path}`;
  return this.test("neq", message ?? defaultMessage, function (value: string) {
    if (isAbsent(value)) return true;
...
export const isMoreThanWithRef = function (
  this: INumericSchema,
...
export const isLessThanWithRef = function (
  this: INumericSchema,
